...
export const mockEcommerce: Project = {
  ...
  difficulties: `Managing state and persistant storage. 
                ...
                to save it, so that when a new page is loaded, the previous state in storage is used to initalize the Redux store. 
                It was very complicated creating the store from storage because the way Next.js 
                handles routing on a staic site is a mix bewteen client side  
...
export const traderBot: Project = {
  name: "Trader Bot",
  description: `Python program to help automate cryptocurrency trades. 
  ...
  solution: `I went with an single threaded event loop approach because it seemed the simplest.  At a high level the 
            program waits for data, and reacts when it arrives, so I used asyncio to create an event loop, 
             gave it tasks that yeild data, and told it to run forver. When a 
             task yeilds data from a websocket it calls a function that starts the trading process.`,
...
export const choreApp: Project = {
  ...
  features: ["Single Page Application", "User authenicatoin", "Json Web Token"],
...
export const rockPaperScissors: Project = {
  ...
  solution: `Keep app layout simple, don't rerender game pieces once on screen.`,
...
export const alienInvasion: Project = {
  ...
  images: [
    {
      ...
      alt: "Screen shot of Alien Invasion gameplay",
    },
  ],
  ...
  solution: `Tweak level difficultly after testing. 
            ...
            rockets with a proton-7 laser temporaraly.`,
  features: ["Old school hard gameplay", "60fps", "Incredible graphics"],
